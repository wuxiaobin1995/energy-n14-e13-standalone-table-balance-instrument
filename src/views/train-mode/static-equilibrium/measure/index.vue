 ...
 * @LastEditTime: 2023-02-02 20:29:29
  ...
  <div class="train-static-equilibrium-measure">
    <div class="wrapper">
      <div class="main">
        ...
        <div class="right">
          <div class="coefficient">训练姿势：{{ trainPosture }}</div>
          <div class="coefficient">晃动系数：{{ coefficient }}</div>
          <div class="coefficient">训练组数：{{ nowNum }} /{{ num }}</div>
...
export default {
  ...
  data() {
    return {
      ...
      nowNum: JSON.parse(this.$route.query.num),
    ...
    save() {
      ...
      db.train_data
        .add({
          ...
          num: JSON.parse(this.$route.query.num), // 训练组数
          time: JSON.parse(this.$route.query.time), // 测量时长
          restTime: JSON.parse(this.$route.query.restTime), // 测量时长
          type: '静态平衡训练'
        })
        .then(() => {
          this.isPdf = true
